...
이 문서는 AnyFive 그룹웨어(office.anyfive.com)의 게시판 데이터 크롤링 + 다운로드 파일을 활용하여 cmds 형식으로 변환하는 전체 과정을 설명합니다. 
...
1. **크롤링**: AnyFive 그룹웨어에서 게시판 데이터(게시글, 댓글) 수집
...
이미지와 첨부파일은 크롤링을 하지 않고 다운로드 파일을 사용합니다.
...
1. **게시판 타입 입력**
   ...
   ```
DB에 게시판별로 테이블이 저장되는것이 아닌, 한 테이블에 여러 게시판이 저장되기 때문에 이를 구분하고자 타입 입력을 받고 있으며, 맨 아래에 타입 이름 예시가 기재되어있습니다.
